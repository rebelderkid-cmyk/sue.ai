...
DATA_STORE_ID = os.getenv("DATA_STORE_ID", "YOUR_DATA_STORE_ID")
...
def retriever(query_str, filters=None):
    ...
    try:
        ...
        # NOTE: Vertex AI Filtering is brittle. We use Robust Python-Side Filtering with Pagination.
        # We fetch up to 300 docs (6 pages x 50) to find matches.
        
        all_results = []
        page_token = None
        MAX_PAGES = 6 # Limit to 300 docs scan
        ...
        for _ in range(MAX_PAGES):
            request = discoveryengine.SearchRequest(
                ...
                query=query_str,
                page_size=50,
                order_by="year desc", # 🆕 Force Sort by Year (Newest First)
            ...
            for result in response.results:
                ...
                # --- Python-Side Filtering (DISABLED TEMPORARILY per User Request) ---
                # if filters:
                #     # 1. Outcome Filter
                #     if filters.get("outcome") and filters["outcome"].strip():
                #         doc_outcome = data.get("outcome", "")
                #         # Exact match
                #         if doc_outcome != filters["outcome"]:
                #             continue
                #     
                #     # 2. Year Filter
                #     if filters.get("year"):
                #         try:
                #             doc_year = int(str(data.get("year", "0")))
                ...
                item = {
                    ...
                    "outcome": data.get("outcome") or "N/A",
                    "text": data.get("full_text") or "No text content found",
                    "pdf_url": pdf_url
                }
...
def answer_synthesizer(user_question, docs):
    ...
    prompt = f"""
    Role: You are a highly skilled **Paralegal Assistant** (ผู้ช่วยทนายความ).
    User: A Lawyer (ทนายความ) who is working on a case.
    Task: Help the lawyer finding relevant Supreme Court Opinions (Deka) to support their case strategy.
    
    Context (Retrieved Deka Cases):
    {context_str}
    ...
    2. **Synthesize**: Summarize key principles from the retrieved cases that match these issues.
    3. **Cite**: Explicitly reference Case IDs (e.g., ฎีกาที่ 123/2566) as authority.
    4. **Assistance**: Suggest how these cases can support the lawyer's argument (Win/Loss factors).
    5. **Tone**: Professional, Concise, Supportive.
    6. **Language**: Primary language is **Thai (Formal Legal)**.
...
def chat_workflow(user_question):
    ...
    # 1. Optimize Query
    print("🤖 (Agent Cloud) Refining Query...")
...
def answer_synthesizer_stream(user_question, docs, history=None):
    ...
    Role: You are a highly skilled **Paralegal Assistant** (ผู้ช่วยทนายความ).
    User: A Lawyer (ทนายความ) who is working on a case.
    Task: Discuss with the lawyer and help them find relevant Supreme Court Opinions (Deka).
    
    Previous Conversation:
    {history_str}
    
    Context (New Retrieved Deka Cases):
    {context_str}
